...
def main():
    parser = argparse.ArgumentParser(
        description="两阶段层次-K-means聚类 — N1 22维新特征集"
    )
    parser.add_argument(
        "--input-npz",
        type=str,
        default=str(Path(__file__).resolve().parent / "N1_decayed_core_22d_clean.npz"),
        help="预提取的 N1 干净数据文件",
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=f"输出根目录 (默认: {DEFAULT_HIERARCHICAL_CLUSTERING_OUTPUT_DIR})",
    )
    parser.add_argument("--k-over", type=int, default=150, help="过度聚类K值 (默认: 150)")
    parser.add_argument("--n-clusters", type=int, default=6, help="最终类别数 (默认: 6)")
    parser.add_argument("--random-state", type=int, default=42, help="随机种子")

    args = parser.parse_args()

    print("=" * 70)
    print("两阶段层次-K-means聚类 — N1 22维新特征集")
    print(f"输入: {args.input_npz}")
    print(f"k_over={args.k_over}, K={args.n_clusters}")
    print("=" * 70)
    ...
    k_range = list(range(3, min(args.n_clusters + 3, 11)))
    results, Z = stage2_hierarchical_clustering(centroids, k_range)

    final_labels = fcluster(Z, args.n_clusters, criterion="maxclust") - 1
    stage1_labels = kmeans_model.labels_
    final_sample_labels = final_labels[stage1_labels]

    unique, counts = np.unique(final_sample_labels, return_counts=True)
    print(f"\n最终 K={args.n_clusters} 类分布:")
    for u, c in zip(unique, counts):
        print(f"  类 {u}: {c:,} ({c/len(final_sample_labels)*100:.1f}%)")

    output_dir = os.path.join(
        args.output_dir, f"cluster_results_n1_K{args.n_clusters}"
    )
    os.makedirs(output_dir, exist_ok=True)

    print("\n生成可视化...")
    plot_dendrogram(Z, os.path.join(output_dir, "dendrogram.png"), k_range)
    plot_evaluation_curves(results, os.path.join(output_dir, "evaluation_curves.png"))

    result_file = os.path.join(output_dir, f"cluster_results_K{args.n_clusters}.npz")
    np.savez_compressed(
        result_file,
        X=X,
        labels=final_sample_labels,
        lat=lat,
        lon=lon,
    ...
    with open(os.path.join(output_dir, "config.json"), "w") as f:
        json.dump({
            "feature_set": "N1_decayed_core_22d",
    ...
    print(f"样本数: {X.shape[0]:,} | 特征维度: {X.shape[1]} | 类别数: {args.n_clusters}")
    print("=" * 70)
